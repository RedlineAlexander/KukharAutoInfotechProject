...
Розробити базу даних, що складається з двох таблиць. Автомобіль, з унікальним ідентифікатором (наприклад номер) 
та декілька полів опису (наприклад марка, модель, колір, рік випуску…) 
та Власник, з унікальним ідентифікатором та  полями (прізвище, ім’я, по батькові, дата народження). 
Зв’язок між таблицями "багато-до-багатьох". Заповнити ці таблиці тестовими даними (10-15 записів в кожній таблиці).
...
5.	( select * from Owners JOIN LEFT OR RIGHT Autos or AutosAndOwners on  Autos.auto.id = Owners.owner.id  owner. id = owner.id )
